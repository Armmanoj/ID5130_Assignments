...
void send_boundary(double* G, int M, int N, int rank, int size, MPI_Comm comm, const int half){
    // half is either 0 (left half sends data) or 1 (right half sends data)
    if (rank == 0) {
        if (half) {
            MPI_Send(G + N * (M - 1) + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank + 1, 1, comm);
        } 
        else {
            MPI_Send(G + N * (M - 1) + 1, N / 2 -1, MPI_DOUBLE, rank + 1, 1, comm);
        }
    } 
    else if (rank == size - 1) {
        if (half) {
            MPI_Send(G + N + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank - 1, 1, comm);
        } 
        else {
            MPI_Send(G + N + 1, N / 2-1 , MPI_DOUBLE, rank - 1, 1, comm);
        }
    }
    else {
        if (half) {
            MPI_Send(G + N * M + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank + 1, 1, comm);
            MPI_Send(G + N + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank - 1, 1, comm);
        } 
        else {
            MPI_Send(G + N * M + 1,  N / 2-1 , MPI_DOUBLE, rank + 1, 1, comm);
            MPI_Send(G + N + 1, N / 2 -1, MPI_DOUBLE, rank - 1, 1, comm);
        }
    }
    return;
...
void recv_boundary(double* G, int M, int N, int rank, int size, MPI_Comm comm, const int half){
    MPI_Status status;
    if (rank == 0) {
        if (!half) {
            MPI_Recv(G + N*M+ 1, N / 2 -1, MPI_DOUBLE, rank + 1, 1, comm, &status);
        } 
        else {
            MPI_Recv(G + N * M + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank + 1, 1, comm, &status);
        }
    } 
    else if (rank == size - 1) {
        if (!half) {
            MPI_Recv(G + 1, N / 2 -1, MPI_DOUBLE, rank-1 , 1, comm, &status);
        } 
        else {
            MPI_Recv(G + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank - 1, 1, comm, &status);
        }
    }
    else {
        if (!half) {
            MPI_Recv(G + N * (M + 1) + 1, N / 2 -1, MPI_DOUBLE, rank + 1, 1, comm, &status);
            MPI_Recv(G + 1, N / 2 -1, MPI_DOUBLE, rank - 1, 1, comm, &status);
        } 
        else {
            MPI_Recv(G + N * (M + 1) + N / 2 , N - N / 2 - 1, MPI_DOUBLE, rank + 1, 1, comm, &status);
            MPI_Recv(G + N / 2, N - N / 2 - 1, MPI_DOUBLE, rank - 1, 1, comm, &status);
        }
    }
    return;
...
void gathererr(double* G, double* Err, int M, int N, int rank, int size, MPI_Comm Comm) {
    ...
    MPI_Gatherv(G, M * N, MPI_DOUBLE, Err, sendcounts, displacements, MPI_DOUBLE, 0, Comm);
    free(sendcounts);
    free(displacements);
    return;
}
...
int Poisson(double* G, double* G_err, double* q, MPI_Comm comm, int rank, int size, int M, int N, double* sol) {
    int iter = 0;
    double err = DBL_MAX;
    double errrecv;
    int row_count;
    ...
    while (err > 0.00000001) {
        if (rank % 2 == 0) {
            for (int i = 1; i < row_count; i++) {
                for (int j = 1; j < N / 2; j++) {
                    int idx = i * N + j;
                    G[idx] = (G[idx-N] + G[N+idx] + G[idx - 1] + G[idx+1] + q[idx-N]) * 0.25;
                }
            }
            send_boundary(G, M, N, rank, size, comm, 0);
            recv_boundary(G, M, N, rank, size, comm, 1);

            for (int i = 1; i < row_count; i++) {
                for (int j = N / 2; j < N - 1; j++) {
                    int idx = i * N + j;
                    G[idx] = (G[(i - 1) * N + j] + G[(i + 1) * N + j] + G[i * N + j - 1] + G[i * N + j + 1] + q[idx-N]) * 0.25;
                }
            }
            for (int i = 0; i < row_count+1; i++) {
                int idx = i * N + (N - 1);
                G[idx] = (4 * G[idx-1] - G[idx-2 ]) / 3;
            }
            send_boundary(G, M, N, rank, size, comm, 1);
            recv_boundary(G, M, N, rank, size, comm, 0);
        } 
        else {
            for (int i = 1; i < row_count; i++) {
                for (int j = N / 2; j < N - 1; j++) {
...
int main(int argc, char** argv)
{
    ...
    int M; // represents the real(without ghost points ) height of array stored in easch processor
    if (N%size){
        if (rank<size-1){
            M = N/size+1;
        }
...
//---------------------------------------------xxx--------------------------------xxx---------------------------------------------------------
    ...
    iter = Poisson(G,G_err,q,MPI_COMM_WORLD,rank,size,M,N,Sol);
    ...
    gatherV(G+N, Sol, M, N,rank, size, MPI_COMM_WORLD);
    double* tempRow = calloc(N,sizeof(double));
    if (rank==0){
        for (int j = 0; j < N; ++j) {
            tempRow[j] = Sol[(N - 1) * N + j];
        }
        for (int i = N - 1; i > 0; --i) {
            for (int j = 0; j < N; ++j) {
                Sol[i * N + j] = Sol[(i - 1) * N + j];
            }
        }
        for (int j = 0; j < N; ++j) {
            Sol[j] = tempRow[j];
