...
void saveDoubleArrayToBinary(const std::vector<std::vector<double>>& Array, const char* filePath) {
    ...
    if (file != NULL) {
        ...
        fclose(file);
        std::cout << "Float array saved to " << filePath << std::endl;
    } else {
        std::cout << "Error: Unable to open file " << filePath << " for writing" << std::endl;
    }
}

int Poisson(vector<vector<double>>& G, vector<vector<double>>& G1,vector<vector<double>>& q,int N, double delta,vector<vector<double>>& G_err){
    int iter = 0;
    double err = MAXFLOAT;
    for (int i=0; i<N; i++){
        for (int j=0; j<N; j++){
    ...
    while (err>0.00000001){
        for (int i=1; i<N-1; i++){
            for (int j=1; j<N-1; j++){
                G1[i][j] = (G[i-1][j]+G[i+1][j]+G[i][j-1]+G[i][j+1]+q[i][j])*0.25;
            }
            G1[i][N-1] = (4*G1[i][N-2]-G1[i][N-3])/3; // In handling boundary, we have to use the same G1, else soltion will blow up 
        }
        G1.swap(G);
        iter++;
...
int main(int argc, char** argv){
    ...
    vector<vector<double>> G1(N, vector<double>(N, 0));
    ...
    int iter = Poisson(G,G1,q,N,delta,G_err);
